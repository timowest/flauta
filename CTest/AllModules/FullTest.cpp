...
int main() {
    ...
    for (int r = 0; r < 5; r++) {
        ...
        Vortex_test(all_inputs[r], "gen/vortex_out_%d.txt" ,(r+1), 1); 

        Turbulence_test(all_inputs[r], "gen/turbulence_out_%d.txt", (r+1), 1);
    
        
        for (int j = 0; j < 5; j++) {
            //float* inputs2[] = {all_inputs[r], all_inputs[j]};  
            
            Bernoulli_test(all_inputs[r], all_inputs[j], "gen/bernoulli_out_%d.txt", (r+1)*10+j+1, 3);

            JetDrive_test(all_inputs[r], all_inputs[j], "gen/jetdrive_out_%d.txt", (r+1)*10+j+1, 2);
     
            Receptivity_test(all_inputs[r], all_inputs[j], "gen/receptivity_out_%d.txt", (r+1)*10+j+1, 1);
